...
class TestCalendarAPI(unittest.TestCase):
    ...
    #     self.assertEqual(response.json()["isRegistered"])
    ...
    def test5_create_event(self):
        response =requests.get(f"{self.url}/create_event", self.login_data, headers=self.headers)

        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.status_code, 500)


    def test6_get_events_by_date(self):
        response = requests.get(f"{self.url}/get_events_by_date/<date>", json=self.login_data, headers=self.headers)

        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.status_code, None)
